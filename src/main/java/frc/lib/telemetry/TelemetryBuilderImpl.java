...
public class TelemetryBuilderImpl implements TelemetryBuilder {
  @FunctionalInterface
  ...
  private static class Property<P extends Publisher, S extends Subscriber>
    ...
    void update(long time) {
      if (m_sub != null && m_updateLocal != null) {
  ...
  // TODO: This probably needs to be revisited a bit, e.g. how to check if
  //  a child node wasn't closed etc.
  private final List<TelemetryBuilder> m_children = new ArrayList<>();
  ...
  @SuppressWarnings("PMD.AvoidCatchingGenericException")
  public void close() {
    for (Property<?, ?> entry : m_properties) {
      entry.close();
    }
  ...
  @Override
  public void bindChild(String name, TelemetryNode node) {
    var builder = new TelemetryBuilderImpl();
    builder.setTable(m_table.getSubTable(name));
    m_children.add(builder);
    node.bind(builder);
    builder.update();
  }

  @Override
  public void bindSendableChild(String name, Sendable sendable) {
    var builder = new TelemetryBuilderImpl();
    builder.setTable(m_table.getSubTable(name));
    m_children.add(builder);
